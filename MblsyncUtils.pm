...
## Make sure homeDirectory is defined
unless ($homeDirectory) {
	die "ERROR: Could not get homeDirectory. $!";
}
...
sub backupChooser {
	my @chosenBackup; ## This will ultimately be the full path to the iOS Backup we will use. 
	...
	#our @iosBackupsUIDonly; ## Just the UID (directory name) of the full path to iOS Backup;
	#our %iosBackupsUIDonly; ## Just the UID (directory name) of the full path to iOS Backup;
	## Default location of iOS Backups
	#if ($macOS) {$iOSbackupRoot = "$ENV{'HOME'}/Library/Application Support/MobileSync/Backup";}
	#if ($MSwin) { $iOSbackupRoot = "$ENV{'APPDATA'}\\Apple Computer\\MobileSync\\Backup"; }

			

	{ ## Begin loop until a directory with at least one iOS backup is found.
...
#				print "uid [$uid] $device_name\n";die;
		...
		# 	print "ffn [$File::Find::name]\n";
		# 	print "ffd [$File::Find::dir]\n";
		# 	print "ff_ [$_]\n";
	
			## If the file we are at is a directory and name looks like an iOS
			## backup UID directory name, then look for an Info.plist within the 
			## directory.  If Info.plist found then assume the parent directory 
...
sub outputDirectoryRoot_user {
	...
	## Loop until user enters a pre existing and writable directory.
	{
...
# 	my $outputDirectoryRoot_user = <STDIN>;
# 	chomp $outputDirectoryRoot_user;
# 	## If user pressed enter and nothing else then return the original outputDirectoryRoot.
# 	if ($outputDirectoryRoot_user eq "") { return $outputDirectoryRoot;}

# 	if ($macOS){	
# 		## Remove single back slashes, but replace double back slashes with a single backslash.
# 		$outputDirectoryRoot_user = macOSRemoveShellEscape($outputDirectoryRoot_user);
# 				
# 		## If user specified a home directory using ~/ then replace with ENV{'HOME'}/
# 		if ($outputDirectoryRoot_user =~ /^~\//) {
# 			$outputDirectoryRoot_user =~ s/^~\//$ENV{'HOME'}\//;
# 		}
# 	}
# 	
# 	return $outputDirectoryRoot_user;
